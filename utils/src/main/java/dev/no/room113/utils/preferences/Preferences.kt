package dev.no.room113.utils.preferences

import android.annotation.SuppressLint
import android.content.Context
import android.content.SharedPreferences
import com.google.gson.Gson
import dev.no.room113.utils.gson.fromJson
import dev.no.room113.utils.gson.toJSONString
...
abstract class Preferences(private val name: String? = null) {
    ...
    inner class GenericPrefDelegate<T : Any>(
        ...
        override fun getValue(thisRef: Preferences, property: KProperty<*>): T? =
            when (type) {
                ...
                StorableType.Object ->
                    prefs.getString(key, null)?.let { Gson().fromJson(it) as T? }
            }
        ...
        override fun setValue(thisRef: Preferences, property: KProperty<*>, value: T?) =
            when (type) {
                ...
                is StorableType.Object -> {
                    val convertedValue = value?.toJSONString()
                    prefs.edit().putString(prefKey ?: property.name, convertedValue)
                        .apply()
                    onPrefChanged(property)
                }
            }
    ...
    fun <T : Any> objectPref(
        prefKey: String? = null,
        defaultValue: T? = null,
    ) = GenericPrefDelegate(prefKey, defaultValue, StorableType.Object)
